<?php
// bot_webhook.php
// Version optimizada del bot para hosting gratuito (via Webhooks)
// No necesita ejecutarse en consola, se dispara cuando Telegram envia un mensaje.
...
// REPORTAR ERRORES (Solo para depuracion)
error_reporting(E_ALL);
ini_set('display_errors', 1);

// Sistema de LOGS para depuracion
file_put_contents('log_bot.txt', "[" . date('Y-m-d H:i:s') . "] Payload recibido: " . $content . "\n", FILE_APPEND);

if (!$update) {
        file_put_contents('log_bot.txt', "[" . date('Y-m-d H:i:s') . "] Error: No se recibio un JSON valido o peticion vacia.\n", FILE_APPEND);
        exit;
}
...
function sendContact($chatId, $text) {
        $keyboard = [
                    'keyboard' => [[['text' => 'Compartir mi numero de telefono', 'request_contact' => true]]],
...
try {
        ...
        if (isset($update['callback_query'])) {
            ...
            if (!$cliente && !in_array($data, ['registrar_nueva', 'check_contact'])) {
                            sendMessage($chatId, "No vinculado. Pulsa /start.");
                            exit;
            }
            ...
            if ($data === 'agendar') {
                            ...
                            if (!empty($pets)) {
                                                $btns = array_map(function($p) {
                                                                        return [['text' => 'Mascota: ' . $p['nombre'], 'callback_data' => "selpet_start_{$p['id']}"]];
                                                }, $pets);
                                                $btns[] = [['text' => 'Agregar nueva mascota', 'callback_data' => "newpet_start"]];
                                                sendMessage($chatId, "Para cual de tus <b>mascotas</b> es la cita?", ['inline_keyboard' => $btns]);
                            } else {
                                                setStep($chatId, 'preg_nombre');
                                                sendMessage($chatId, "Entendido! Vamos a registrar a tu mascota. \n\nCual es su <b>nombre</b>?");
                            }
                            ...
                            $btns = array_map(function($v) {
                                                return [['text' => 'Vet: '.$v['nombre'], 'callback_data' => 'vet_'.$v['id']]];
                            }, $vets);
                            sendMessage($chatId, "Selecciona un <b>veterinario</b>:", ['inline_keyboard' => $btns]);
            } elseif ($data === 'newpet_start') {
                            setStep($chatId, 'preg_nombre');
                            sendMessage($chatId, "Cual es el <b>nombre</b> de la nueva mascota?");
            } elseif (substr($data, 0, 4) === 'vet_') {
                            if (!$estado) { sendMessage($chatId, "Sesion expirada. Pulsa /start."); exit; }
                            $vid = substr($data, 4);
                            $sdata = $estado['data'] ?? [];
                            $sdata['vid'] = $vid;
                        ...
                        if (!$estado || !isset($estado['data']['vid'])) { sendMessage($chatId, "Sesion expirada. Pulsa /start."); exit; }
                        $fecha = substr($data, 2);
                        $sdata = $estado['data'];
                        $sdata['f'] = $fecha;
                        ...
                        if (!$estado || !isset($estado['data']['f'])) { sendMessage($chatId, "Sesion expirada. Pulsa /start."); exit; }
                        $hora = substr($data, 2);
                        $sdata = $estado['data'];
                        $db = getDB();
                        ...
                        if ($stmtC->execute([$cliente['id'], $sdata['vid'], $sdata['mid'], $sdata['f'], $hora])) {
                                            ...
                                            clearStep($chatId);
                                            sendMessage($chatId, "<b>Cita agendada con exito!</b>\n\n<b>Fecha:</b> {$sdata['f']}\n<b>Hora:</b> $hora\n<b>Mascota:</b> $pname\n\n<i>Nota: El precio final depende de los servicios realizados, medicamentos o terapias necesarias durante la consulta.</i>\n\nTe esperamos!");
                        } else {
                        ...
                        sendMessage($chatId, "Genial! Vamos a registrarte. \n\nCual es tu <b>nombre completo</b>?");
        } elseif ($data === 'check_contact') {
                        sendContact($chatId, "Por favor, presiona el boton de abajo para compartir tu numero:");
        }
} elseif (isset($update['message'])) {
            $msg = $update['message'];
        $text = trim($msg['text'] ?? '');
        ...
        if ($text === '/start') {
                        if ($cliente) {
                                            ...
                                            $kb = ['inline_keyboard' => [
                                                                    [['text' => 'Compartir Numero (Si ya eres cliente)', 'callback_data' => 'check_contact']],
                                                                    [['text' => 'Registrarme como nuevo cliente', 'callback_data' => 'registrar_nueva']]
                                                                ]];
                                            sendMessage($chatId, "Bienvenido a la Clinica Veterinaria! \n\nVeo que aun no estas registrado. Como deseas continuar?", $kb);
                        }
                        clearStep($chatId);
                        exit;
        } elseif ($text === '/cancel' || strtolower($text) === 'cancelar') {
                        clearStep($chatId);
                        sendMessage($chatId, "Accion cancelada.");
        ...
        if (isset($msg['contact'])) {
                        ...
                        if ($cl) {
                                            ...
                                            sendMessage($chatId, "Numero no registrado en nuestro sistema.");
                        }
                        exit;
        }
        ...
        if ($estado) {
                        ...
                        if ($step === 'reg_nombre') {
                                            ...
                                            sendMessage($chatId, "Perfecto, <b>$text</b>. Ahora, cual es tu <b>numero de telefono</b>?");
                                            ...
                                            if ($stmt->execute([$sdata['nombre'], $text, $chatId])) {
                                                                    clearStep($chatId);
                                                                    sendMessage($chatId, "Registro completado con exito!");
                                            ...
                                            sendMessage($chatId, "Entendido. Que <b>animal</b> es $text? (Perro, gato, conejo, etc.)");
                        } elseif ($step === 'preg_especie') {
                                            $sdata['m_especie'] = $text;
                                            setStep($chatId, 'preg_raza', $sdata);
                                            sendMessage($chatId, "De que <b>raza</b> es?");
                        } elseif ($step === 'preg_raza') {
                                            $sdata['m_raza'] = $text;
                                            setStep($chatId, 'preg_edad', $sdata);
                                            sendMessage($chatId, "Que <b>edad</b> tiene?");
                        } elseif ($step === 'preg_edad') {
                                            $sdata['m_edad'] = $text;
                                            setStep($chatId, 'preg_vacunas', $sdata);
                                            sendMessage($chatId, "Tiene sus <b>vacunas</b> al dia? (Si/No, cuales)");
